...
def hough_lines(image):                                                                     # trả về tập hợp các cạnh phát hiện được
    ...
    return cv.HoughLinesP(image, cv.HOUGH_PROBABILISTIC, theta=np.pi/1, threshold=0, minLineLength=10, maxLineGap=10)
...
img = cv.imread("Parking_Lot_NonDL/dataset/extend2.png")
...
for i in range(len(clusters)):
    j = 1
    while j < len(clusters[i]):
        if check_object_in_slot(clusters[i][j - 1], clusters[i][j], overlay):  # true if empty slot, draw green rec
